...
const getParameterFromSSM = async function(): Promise<string> {
  return (await ssmClient.send(new GetParameterCommand({Name: keyParameterName, WithDecryption: true}))).Parameter?.Value || '';
}
...
const decodeAndVerifyBearerToken = async function(token: string): Promise<UserAuthorizationPayload> {
  const secretKey = await getParameterFromSSM();
  const decodedPayload = verify(token, secretKey, { complete: true }).payload as UserAuthorizationPayload;
...
const validateUserToken = async function(payload: UserAuthorizationPayload): Promise<boolean> {
  const userItem = (await documentClient.send(new GetCommand({TableName: tableName, Key: {
    recordType: 'USER',
...
export const authorizerHandler = async (event: APIGatewayTokenAuthorizerEvent): Promise<AuthResponse> => {
